...
if __name__ == '__main__':
    ...
    for edge in edge_data:
        fr1 = str(edge_data[edge]['from'])
        to1 = str(edge_data[edge]['to'])
        id1 = str(edge_data[edge]['id'])
        # linkFROM = str(edge_data[edge]['from'])
        # linkID = str(edge_data[edge]['id'])
        # linkTO = str(edge_data[edge]['to'])    
        for edge2 in edge_data:
            fr2 = str(edge_data[edge2]['from'])
            to2 = str(edge_data[edge2]['to'])
            id2 = str(edge_data[edge2]['id'])
            if fr1 == fr2 and to1 == to2 and id1 != id2:
                print('Two interfaces')
                print(str(node_data[fr1]['label'])+'-'+str(node_data[to1]['label']), id1, str(edge_data[edge]['label']))
                
                # print(str(node_data[fr2]['label'])+'-'+str(node_data[to2]['label']), id2, str(edge_data[edge2]['label']))
            # else:
                # print('Keep looking')
